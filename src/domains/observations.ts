...
/**
 * Filters for v2 chart data endpoint.
 */
export interface ChartDataFilters {
  /** Start time in ISO8601 format */
  start: string;
  /** End time in ISO8601 format. Defaults to now if not provided. */
  end?: string;
}
...
export class ObservationsDomain extends BaseClient {
   ...
   */
  async getActiveWis2Stations(): Promise<
   ...
   */
  async getActiveIemStations(): Promise<
    LayerResponse<FeatureCollection<WeatherStationProps>>
  > {
    // v1 only - not yet migrated to v2
    const url = this.resolveUrl({
  ...
  async getStationObservations(
    stationId: string,
    filters: StationObservationFilters,
  ): Promise<StationObservationsResult> {
    // v1 only - not yet migrated to v2
    const url = this.resolveUrl({
  ...
  async getLatestObservation(
    stationId: string,
    provider: ObservationProvider,
  ): Promise<LatestObservationResult> {
    // v1 only - not yet migrated to v2
    const url = this.resolveUrl({
   ...
   */
  async getObservationsByBbox(
    filters: BboxFilters,
  ): Promise<BboxObservationsResult> {
    // v1 only - not yet migrated to v2
    const url = this.resolveUrl({
  ...
  async getStats(
    provider?: ObservationProvider,
  ): Promise<ObservationStatsResult> {
    // v1 only - not yet migrated to v2
    const url = this.resolveUrl({
  ...
  async getChartData(
    ...
    const url = this.resolveUrl({
      v1: null, // v2 only endpoint
      v2: `/observations/${stationId}/chart`,
    });

    const params: Record<string, string> = { start: filters.start };
    if (filters.end) params.end = filters.end;

    const data = await this.get<ChartResponse>(url, params);
